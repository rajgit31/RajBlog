...
<asp:PlaceHolder runat="Server" ID="phAddComment">
    <div id="comment-form">
        ...
        <div class="commentForm">
            ...
            <label for="<%=txtCaptcha.ClientID %>">
                Captcha*</label>
            <img src="/Captcha.aspx" alt="Click to change captcha" style="width: 82px;
                height: 23px" onclick="this.src=RefreshCaptcha(this.src)" />
            <asp:TextBox runat="Server" ID="txtCaptcha" TabIndex="6" MaxLength="4" Width="60px"
                onblur="DoCheckCaptcha()" />
            <span id="CaptchaMsg"></span>
            <asp:RequiredFieldValidator ID="RequiredFieldValidator4" runat="server" ControlToValidate="txtCaptcha"
                ErrorMessage="<%$Resources:labels, required %>" Display="dynamic" ValidationGroup="AddComment" />
            <br />
            ...
            <!--27 -->
            <input type="checkbox" id="cbHuman" style="width: auto" tabindex="7" />
            <label for="cbHuman" style="width: auto; float: none; display: inline">
                I am a human</label><br />
            <br />
            <!--27 -->
    ...
    <script type="text/javascript">
        ...
        function btnSaveAjaxOnClick() {
            if (!$('cbHuman').checked) {
                alert('Please check your inputs and try again. If all fields are correctly entered then ensure \'I am a human\' check box is selected.');
            }
            else {
                if (Page_ClientValidate('AddComment') && checkCaptchaResult) { BlogEngine.validateAndSubmitCommentForm() }
            }
        }
